  ...
  $(document).ready(function() {
    ...
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        ...
        firebase
          .database()
          .ref(`USERS/${uid}/UserProffession`)
          .once('value', snapshot => {
            let profession = snapshot.val();
            neonLogin.setPercentage(100);

            if (profession === 'Teacher') window.location.href = '/';
            else if (profession === 'Student') {
              console.warn('Student logged in');
              alert('Should Redirect to student page...');
            } else {
              let errorMsg = `User is neither teacher nor student. Profession: ${profession}`;
              alert(errorMsg);
    ...
    $.extend(neonLogin, {
      setPercentage: function(pct, callback) {
        ...
        var o = {
          pct: parseInt(
            (neonLogin.$login_progressbar.width() / neonLogin.$login_progressbar.parent().width()) *
              100,
            10
          )
        };
    ...
    if (is_lockscreen) {
      ...
      neonLogin.$lockscreen_progress_canvas
        .attr({
          width: thumb_size,
          height: thumb_size
        })
